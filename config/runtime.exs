...
# Web endpoint. Opt-in and loopback-only.
#
# SpruceGoose is CLI-first; nothing binds a port unless the MCP or admin UI is
# switched on. OAuth requirements remain scoped to MCP rather than forcing an
# unused token service onto the read-only administrator dashboard.
# The bind address is deliberately not configurable from the environment:
# exposing either web interface beyond loopback is a separate, governed
# security decision, not an ops toggle.
mcp_enabled? = System.get_env("SPRUCE_GOOSE_MCP_ENABLED", "false") in ["1", "true"]
admin_enabled? = System.get_env("SPRUCE_GOOSE_ADMIN_ENABLED", "false") in ["1", "true"]
web_enabled? = mcp_enabled? or admin_enabled?

config :spruce_goose, :start_web_endpoint, web_enabled?

if web_enabled? do
  secret_key_base =
    System.get_env("SECRET_KEY_BASE") ||
      raise "SECRET_KEY_BASE is required when a SpruceGoose web interface is enabled"
